...
// 매크로 페이지 - 시간표 뷰어 + 수강신청 매크로 실행 — michael
...
import SelectedCoursesMacro from "@/components/sidebar/SelectedCoursesMacro";
...
async function fetchCourses(): Promise<CoursesApiResponse> {
  console.log("[Macro] 과목 데이터 페칭 시작");
  const res = await fetch("/api/courses");
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    console.error("[Macro] 과목 데이터 페칭 실패:", err);
    throw new Error(err.error ?? "데이터 로드 실패");
  }
  const data = await res.json();
  console.log(`[Macro] 과목 데이터 수신: ${data.total ?? 0}개`);
  return data;
}
...
export default function MacroPage() {
  ...
  const handleToggle = useCallback(
    (courseId: string) => {
      ...
      if (!course) {
        console.warn(`[Macro] 과목 ID 없음: ${courseId}`);
        return;
      }
      toggleCourse(course);
    },
    ...
    <main className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-screen-xl mx-auto px-4 py-3">
          <h1 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
            서이초 방과후 시간표 뷰어
            <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300">
              매크로
            </span>
          </h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            과목을 탭해서 담고, 수강신청 매크로를 실행하세요
          </p>
        </div>
      </header>
      ...
      <div className="max-w-screen-xl mx-auto px-1.5 sm:px-4 py-2 space-y-2">
        ...
        <div className="flex flex-col lg:flex-row gap-4 lg:items-start">
          ...
          {/* 매크로 전용 사이드바 */}
          <SelectedCoursesMacro allCourses={allCourses} />
        </div>
      </div>
    </main>
  );
